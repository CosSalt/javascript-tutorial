...
如果第二个参数不是数值，会被自动转为一个整数。这个整数只有在2到36之间，才能得到有意义的结果，超出这个范围，则返回`NaN`。如果第二个参数转化为数值后是0、NaN或绝对值大于等于Math.pow(2,32)的数，则直接忽略。
...
parseInt('10', 0) // 10
parseInt('10', NaN) // 10
parseInt('10', Math.pow(2, 32)) // 10
parseInt('10', Math.pow(2, 32) - 1) // NaN
parseInt('10', -Math.pow(2, 32)) // 10
parseInt('10', -Math.pow(2, 32) + 1) // NaN
